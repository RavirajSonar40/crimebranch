...
export default function EscalationsPage() {
  ...
  const fetchEscalations = async (user: any) => {
    try {
      setIsSearching(true);
      ...
      console.log('Fetching escalations with params:', params.toString());
  ...
  const fetchFilters = async (user: any) => {
    try {
      const response = await fetch(`/api/escalations/filters?userId=${user.user_id}&role=${user.role}`);
      if (response.ok) {
        const data = await response.json();
    ...
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black relative">
          ...
          <div className="bg-gray-900 rounded-xl p-8 border border-gray-800 shadow-2xl">
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                <div className="text-white text-xl font-medium animate-pulse">Searching Escalations...</div>
      ...
      <div className="bg-gray-900/50 backdrop-blur-sm border-b border-gray-800 p-4">
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-bold text-white">Escalations Management</h1>
      ...
      <div className="flex">
        ...
        <div className="flex-1 p-6">
          <div className="space-y-6">
            <h2 className="text-3xl font-bold text-white mb-6">All Escalations</h2>
                  ...
                  <span className="relative z-10 flex items-center gap-2">
                    {isSearching ? (
                      ...
                      <>
                        ...
                        <span className="group-hover:font-semibold transition-all duration-200">Search Escalations</span>
                      </>
                    )}
                  </span>
                </button>
            ...
            <div className="bg-gray-900/50 backdrop-blur-sm rounded-xl border border-gray-800 overflow-hidden">
              <div className="p-6 border-b border-gray-800">
                <h3 className="text-xl font-semibold text-white">
                  Escalations ({escalations.length} found)
                </h3>
              </div>
              ...
              {loading ? (
                <div className="p-6 text-center">
                  <div className="flex items-center justify-center gap-3">
                    ...
                    <div className="text-white text-lg">Loading escalations...</div>
                  </div>
                </div>
              ) : escalations.length === 0 ? (
                <div className="p-6 text-center">
                  <div className="text-gray-400">No escalations found matching your filters.</div>
                </div>
              ) : (
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="bg-gray-900/30 divide-y divide-gray-800">
                      {escalations.map((escalation) => (
                        <tr key={escalation.escalation_id} className="hover:bg-gray-800/50">
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-white">
                            #{escalation.escalation_id}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-white">
                            {escalation.crime_title}
                          </td>
